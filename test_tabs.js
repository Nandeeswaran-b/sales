...
global.setTimeout = setTimeout;
    ...
    window.onerror = function(message, source, lineno, colno, error) {
        ...
        errorDiv.style.padding = '14px 20px';
        ...
        errorDiv.style.borderLeft = '5px solid #7f1d1d';
        errorDiv.innerHTML = `<strong>Frontend Runtime Error:</strong><br>${message}<br><br><small>Line ${lineno}:${colno} in ${source.split('/').pop()}</small>`;
    ...
    async function pingUntilAlive() {
        ...
        const MAX_TRIES = 12; // 60 seconds max
        for (let i = 0; i < MAX_TRIES; i++) {
            try {
                const controller = new AbortController();
                const timeoutId = setTimeout(() => controller.abort(), 6000);
                const res = await fetch(API_BASE + '/api/summary', { signal: controller.signal });
            ...
            if (banner) banner.querySelector('span').textContent = `Connecting to server... (${i+1}/12)`;
            await new Promise(r => setTimeout(r, 5000));
        }
        if (banner) {
            banner.style.background = 'rgba(239,68,68,0.15)';
            banner.style.borderColor = 'rgba(239,68,68,0.4)';
            banner.querySelector('span').textContent = 'Server unreachable. Please refresh the page.';
        }
        return false;
    }
    ...
    // Initial routing setup — wait for server to wake up first, but proceed anyway if offline
    (async () => {
        await pingUntilAlive();
